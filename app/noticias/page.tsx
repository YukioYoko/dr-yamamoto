...
export default function Noticias() {
  const [selectedFilter, setSelectedFilter] = useState<'all' | 'event' | 'news' | 'achievement'>('all');
  const [posts, setPosts] = useState<Post[]>(staticPosts); // Inicializamos con los estáticos
  ...
  useEffect(() => {
    async function fetchPosts() {
      try {
        ...
        console.warn('No se pudo conectar a la API. Usando datos estáticos por defecto.');
        // Si falla (n8n/webhook inactivo), los datos estáticos ya están asignados.
      } finally {
        setLoading(false);
      }
    }
